...
Answer: It has an evidence-gated abstention step: before generating, it checks the best retrieved evidence score, and if it's below a calibrated threshold it refuses instead of confabulating — provider-agnostic, so it guards hosted models too. I measure it honestly with refusal precision and recall over a set of unanswerable golden questions. On my corpus it gets precision 1.0 (it never falsely refuses a real question) and recall 0.5. The interesting part is *why* recall is 0.5: off-domain questions retrieve near-zero scores and are caught perfectly, but in-domain-but-unanswerable questions — "what's the maximum fine under the EU AI Act?" — pull a topically-related chunk that scores as high as a real question, so a lexical score threshold can't separate them. Forcing recall up would start false-refusing real questions. The principled fix is a semantic / NLI entailment check behind the same metrics, which I scoped as the next step rather than overclaiming. I also gate CI on these metrics, so if abstention silently breaks, recall drops to 0 and the build fails.
...
- Added evidence-gated abstention to the RAG writer (refuse when retrieval confidence is low) and measured it with refusal precision/recall over unanswerable questions, gating CI on both so abstention regressions fail the build.
